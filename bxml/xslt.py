...
XSL_TEMPLATE = (
    """<xsl:stylesheet version="%s" %s%s><xsl:output method="xml"/></xsl:stylesheet>"""
)
...
class XSLT(XML):
    ...
    def saxon6(self, elem, **params):
        ...
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            if (
                self.find(self.root, "xsl:output") is None
                or self.find(self.root, "xsl:output").get("method") == "xml"
            ):
    ...
    def copy_select(cls, select):
        elem = XML.Element(
            """<xsl:copy %s select="%s"></xsl:copy>""", XSL_NAMESPACE, select
        )
        for val in vals:
            elem.append(val)
        return elem
    ...
    def when(cls, test, *vals):
        elem = XML.Element(
            """<xsl:when %s test="%s"></xsl:when>""", XSL_NAMESPACE, test
        )
        for val in vals:
            elem.append(val)
        return elem
    ...
    def otherwise(cls, *vals):
        elem = XML.Element(
            """<xsl:otherwise %s></xsl:otherwise>""", XSL_NAMESPACE, test
        )
        for val in vals:
            elem.append(val)
        return elem
    ...
    def template_match(cls, match, *vals):
        elem = XML.Element(
            """<xsl:template %s match="%s"></xsl:template>""", XSL_NAMESPACE, match
        )
        for val in vals:
            elem.append(val)
        return elem
    ...
    def template_name(cls, name, *vals):
        elem = XML.Element(
            """<xsl:template %s name="%s"></xsl:template>""", XSL_NAMESPACE, name
        )
        for val in vals:
            elem.append(val)
        return elem
    ...
    def apply_templates_mode(cls, mode):
        return XML.Element(
            """<xsl:apply-templates %s mode="%s"/>""", XSL_NAMESPACE, mode
        )

    @classmethod
    def apply_templates_select(cls, select):
        return XML.Element(
            """<xsl:apply-templates %s select="%s"/>""", XSL_NAMESPACE, select
        )
    ...
    def element(cls, name, *vals):
        elem = XML.Element(
            """<xsl:element %s name="%s"></xsl:element>""", XSL_NAMESPACE, name
        )
        for val in vals:
            elem.append(val)
        return elem
    ...
    def attribute(cls, name, *vals):
        elem = XML.Element(
            """<xsl:attribute %s name="%s"></xsl:attribute>""", XSL_NAMESPACE, name
        )
        for val in vals:
            elem.append(val)
        return elem
    ...
    def variable(cls, name, *vals):
        elem = XML.Element(
            """<xsl:variable %s name="%s"></xsl:variable>""", XSL_NAMESPACE, name
        )
        for val in vals:
            elem.append(val)
        return elem
    ...
    def variable_select(cls, name, select):
        return XML.Element(
            """<xsl:variable %s name="%s" select="%s"/>""", XSL_NAMESPACE, name, select
        )
    ...
    def for_each(cls, select, *vals):
        elem = XML.Element(
            """<xsl:for-each %s select="%s"></xsl:for-each>""", XSL_NAMESPACE, select
        )
        for val in vals:
            elem.append(val)
        return elem
    ...
    def output_method(cls, method):
        return XML.Element(
            """<xsl:output method="%s" encoding="utf-8" %s/>""", XSL_NAMESPACE, method
        )
