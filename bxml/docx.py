...
class DOCX(ZIP):
    ...
    def transform(self, transformer, fn=None, XMLClass=None, **params):
        return self.xml(fn=fn).transform(
            transformer=transformer, XMLClass=XMLClass, docx=self, **params
        )
    ...
    def stylemap(self, definitions=True, all=True, cache=False):
        ...
        if self._stylemap is not None and cache == True:
            ...
            for s in x.root.xpath("w:style", namespaces=self.NS):
                ...
                style.xpath = (
                    "//w:rStyle[@w:val='%(id)s'] | //w:pStyle[@w:val='%(id)s']" % style
                )
                if all == False:
                    ...
                    if len(uses) == 0:
                        ...
                        if len(uses) == 0:
                            if endnotes is not None:
                                uses = endnotes.root.xpath(
                                    style.xpath, namespaces=DOCX.NS
                                )
                            if len(uses) == 0:
                                continue
    ...
    @classmethod
    def val_to_css(C, val, factor, unit=CSS.rem, pt_per_em=12.0, decimals=2):
    ...
    def selector(C, style):
        ...
        if style.type == "paragraph":
            ...
            outlineLvl = (
                int((style.properties.get("outlineLvl") or {}).get("val") or 8) + 1
            )
            if outlineLvl < 9:
                tag = "h%d" % outlineLvl
            else:
    ...
    def stylesheet(
        ...
        for styleName in used_styles:
            ...
            css.styles[sel] = self.style_properties(
                styles,
                styleName,
                unit=unit,
                pt_per_em=pt_per_em,
    ...
    def style_properties(
        ...
        for j in style.properties.keys():
            ...
            if j == "spacing":
                for k in prop.keys():
                    if k == "after":
                        properties["margin-bottom:"] = self.val_to_css(
                            prop[k], factor=space_factor, unit=unit, pt_per_em=pt_per_em
                        )
                    elif k == "before":
                        properties["margin-top:"] = self.val_to_css(
                            prop[k], factor=space_factor, unit=unit, pt_per_em=pt_per_em
                        )
                    elif k in ["beforeAutospacing", "afterAutospacing"]:
                        pass
                    elif k in ["line", "val"]:
                        properties["line-height:"] = self.val_to_css(
                            prop[k], factor=space_factor, unit=unit, pt_per_em=pt_per_em
                        )
                    elif k == "lineRule":
                        pass
                ...
                for k in prop.keys():
                    if k == "val":
                        properties["font-size:"] = self.val_to_css(
                            prop[k], factor=font_factor, unit=unit, pt_per_em=pt_per_em
                        )
                    else:
                ...
                for k in prop.keys():
                    if k == "firstLine":
                        properties["text-indent:"] = self.val_to_css(
                            prop[k], factor=space_factor, unit=unit, pt_per_em=pt_per_em
                        )
                    elif k == "hanging":
                        properties["text-indent:"] = self.val_to_css(
                        ...
                        properties["margin-left:"] += self.val_to_css(
                            prop[k], factor=space_factor, unit=unit, pt_per_em=pt_per_em
                        )
                    elif k == "right":
                        properties["margin-right:"] = self.val_to_css(
                            prop[k], factor=space_factor, unit=unit, pt_per_em=pt_per_em
                        )
                    elif k == "hanging":
                        if properties.get("margin-left:") is None:
